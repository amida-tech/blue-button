...
*/
    ...
    var MedicationActivity = component.define("MedicationActivity")
        ...
        .cleanupStep(function (value) {
            if (value) {
                value.identifiers = _.filter(value.identifiers, function (identifier) {
                    if (identifier === null) {
                        return false;
                    } else {
                        return true;
            ...
            /*
          // 1.  startDate --- endDate
          var range = value.times.filter(function(t){
            return -1 === ['PIVL_TS', 'EIVL_TS'].indexOf(t.js.xsitype);
          });

          // 2.  dosing interval
          var period= value.times.filter(function(t){
            return -1 !== ['PIVL_TS', 'EIVL_TS'].indexOf(t.js.xsitype);
          });
          ...
          if (range.length > 0) {
            value.dateRange = range[0];
          }

          if (period.length > 0) {
            value.dosePeriod = period[0].js.period;
          }*/
