...
var lookupReference = function (node, id) {
    ...
    // if no contained text, check for sibling text
    var siblingTextNode = xmlUtil.xpath(resolved[0], "./following-sibling::text()[1]")[0];
    var siblingText = siblingTextNode && processor.asString(siblingTextNode);
...
var resolveReference = cleanup.resolveReference = function () {
    ...
    var ret = null;
    // TODO keep from native text if provided. can check at top
    if (resolved) {
        ret = resolved;
    } else {
