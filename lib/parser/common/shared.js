...
email.cleanupStep(function (value) {
    if (value && value.address) {
        value.address = value.address.substring(7);
...
phone.cleanupStep(function (value) {
    if (value && value.number) {
        if (value.number.substring(0, 4) === "tel:") {
            value.number = value.number.substring(4);
        }
    }
    return value;
});
