...
var documentKeyMapper = getJSON('./sections/unfoundKeyMaps/document.json');
var fs = require('fs');
...
//could possibly use this for inside convertToBBModel's for loop
function processBBSection(data, sectionFunction){
...
function convertToBBModel(intermediateObj){
  ...
  for(var key in intermediateObj){
    ...
    if(sectionProcessFunction){
      console.log("process ection func:" + key);
