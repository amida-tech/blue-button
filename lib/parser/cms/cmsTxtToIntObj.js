...
function normalizeString(cmsString){
    var buffer = new Buffer(cmsString);
    var encodingType = jschardet.detect(buffer).encoding;

    //for now we are going to assume utf 16.
    if(encodingType.toLowerCase() !== 'utf-8' || encodingType.toLowerCase() !== 'ascii'){
        cmsString = buffer.toString('utf16le');
    }
    return cmsString;
    //okay well it should normalize the string, but for now,
}


function getIntObj(cmsString) {
    cmsString = normalizeString(cmsString);
    //console.log(Buffer.isEncoding('utf16le'));
    //console.log(cmsString);
    //console.log(JSON.stringify(buffer.toString('utf16le'), null, 4));
