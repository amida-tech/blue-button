...
function separateSections(data){
  ...
  // 1st regular expression for meta
  ...
  var totalMatchCode = metaMatchCode + "|"+ claimsMatchCode+ "|"+sectionMatchCode;
...
function convertToObject(sectionString){
  ...
  if(sectionTitle.toLowerCase().indexOf("mymedicare") >= 0){
...
function getClaimsBody(sectionString){
  ...
  //or go to the end of the string
  var objectStrings = sectionString.match(claimRegExp);
