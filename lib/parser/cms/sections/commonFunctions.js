...
function cda_line_address(addressString) {
    ...
    if (addressArray.length === 1) {
        ...
        if(fiveDigitZip in zipDict){
            var foundCity = zipDict[fiveDigitZip][0];
            var uppCaseAdrStr = addressString.toUpperCase();
            var foundCityIndex = uppCaseAdrStr.indexOf(foundCity);
            console.log(foundCityIndex);
            if(foundCityIndex >= 1 ){
                var addressLine = addressString.substring(0, foundCityIndex - 1);
                addressObj.streetLines.push(addressLine);
                addressObj.city = addressString.substring(foundCityIndex, foundCityIndex + foundCity.length);
                addressObj.zip = tempArr[tempArr.length -1];
                addressObj.state = tempArr[tempArr.length -2];
            }
    ...
    /*case when there is only one comma, right after the city , ie. 1111 Address Dr Hoosville, VA 11111*/
        ...
        //means that we do start with a street number, and need to look for a common suffix
            ...
            if (!streetFound) {
                ...
                if (zip in zipDict) {
                    var cityRetrieved = zipDict[zip][0];
                    var capAdrString = addressString.toUpperCase();
                    var possibleBreakPoint = capAdrString.indexOf(cityRetrieved);
                    if (possibleBreakPoint >= 1) {
                        breakPoint = possibleBreakPoint - 1;
                    }
                }
        ...
        addressObj.zip = zip;
        if (test) {
            console.log(JSON.stringify(addressString), null, 4);
            console.log(JSON.stringify(addressObj, null, 4));
        }
        return addressObj;
    }
