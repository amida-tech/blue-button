    ...
    if (addressArray.length === 2) {
        ...
        if (isNaN(streetArray[0])) {
            for (var x = streetArray.length; x > 0; x--) {
                ...
                if (!isNaN(streetArray[x])) {
                    streetFound = true;
                    var foundWord = streetArray[x];
                    console.log(foundWord);
                    var breakPoint = streetAndCity.indexOf(foundWord) + foundWord.length;
        ...
        //means that we do start with a street number, and need to look for a common suffix
            ...
            //go through the string and try to look for a usps C1 suffix, streets
            //go through the string and look up usps C2 suffix address

        }
        //need something not computationally expensive.

        /*this is the last option avaliable, when all other methods are exhausted
        assumes that cities are only one words */
