...
var modelTemplate = getJSON('./sections/Models/problems.json');
var sectionUnfoundKeys = getJSON('./sections/unfoundKeyDict/problemsDict.json');
var commonModelUnfoundKeys = getJSON('./sections/unfoundKeyDict/commonModelDict.json');
...
function parseSection(intObj){ //can probably refactor this out
  ...
  if(intObj instanceof Array){ // start recursing thorugh the array of objects
    ...
    for(var x = 0; x < intObj.length; x++){
      var newChild = parseSection(intObj[x]);
    ...
    for(var intKey in intObj){
        ...
        if (!(intKey in modelTemplate) && intKey in sectionUnfoundKeys){
          bbKey = sectionUnfoundKeys[intKey];
        }
        ...
        if(bbKey in modelTemplate){
          ...
          var writeValueResult = writeValue(intKey, valueOfIntObj, valueTemplate, unfilledKeys, valueIsArray);//this needs to be changed to accomodate different functions later
          ...
          if(!templateComplete){
            ...
            if (bbKey in newsectionModel){//if there's already an object in the array
              newsectionModel[bbKey].push(finalValue['template']);
            }
            else{//otherwise make a new array
              newsectionModel[bbKey] = [];
              newsectionModel[bbKey].push(finalValue['template']);
            }
            delete unfinishedValueTemplates[bbKey];
          }
          else if(valueType instanceof Object && !(valueType instanceof Array)){
            newsectionModel[bbKey] = finalValue['template'];
            delete unfinishedValueTemplates[bbKey];
          }
        }
        else{
...
function valueModelFinder(objValue, recursionDepth){
  //console.log("value model: " + objValue);
  if(objValue instanceof Array){
...
//right now this function is only good for demographics
//writes the value onto the given template, returns whether the template is complete and the associated object
function writeValue(intKey, valueOfIntObj, valueTemplate, unfilledKeys, valueIsArray){
  ...
  /*case 1, the object has a field and a required section. In this case,
   ...
   if(valueTemplate in commonModelExceptions){
    ...
    for(var key in valueTemplate){
      if(key in defaultValues && valueIsArray){ ///////////////////NEED TO MODIFY THIS PART TO BECOME MORE ACCOMODATING
