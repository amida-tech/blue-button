...
module.exports = function (data, codeSystems, isCCD, CCDxml) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        xmlDoc = xmlDoc.node('entry').attr({ typeCode: "DRIV" })
            .node('act').attr({ classCode: "ACT" }).attr({ moodCode: "EVN" })
                ...
                xmlDoc = xmlDoc.node('code')
                    ...
                    .attr({ displayName: "Payment sources" }).parent()
                ...
                xmlDoc = xmlDoc.node('entryRelationship').attr({typeCode: "COMP"})
                    .node('act').attr({ classCode: "ACT" }).attr({ moodCode: "EVN" })
                        ...
                        xmlDoc = libCCDAGen.performerRevised(xmlDoc, data[i]["policy"]["insurance"]["performer"], { "typeCode": "PRF", "tID": "2.16.840.1.113883.10.20.22.4.87" });
        ...
        xmlDoc = xmlDoc.node('participant').attr({ typeCode: "COV" })
            ...
            .node('time')
                .node('low').attr(data[i]["participant"]["date_time"]["low"] ? { value: data[i]["participant"]["date_time"]["low"] } : { nullFlavor: "UNK" }).parent()
                .node('high').attr(data[i]["participant"]["date_time"]["high"] ? { value: data[i]["participant"]["date_time"]["high"]} : { nullFlavor: "UNK"}).parent()
            .parent()
            .node('participantRole').attr({ classCode: "PAT"}); 
        ...
        xmlDoc = xmlDoc.node('entryRelationship').attr({typeCode: "REFR"})
            .node('act').attr({ classCode: "ACT"}).attr({ moodCode: "EVN"})
                ...
                .node('procedure').attr({classCode: "PROC"}).attr({moodCode: "PRMS"})
                    xmlDoc = libCCDAGen.code(xmlDoc, data[i]["authorization"]["procedure"] ? data[i]["authorization"]["procedure"]["code"] : undefined).parent()
                xmlDoc = xmlDoc.parent()
            .parent()
        .parent()
    .parent()
