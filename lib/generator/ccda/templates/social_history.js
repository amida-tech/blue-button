...
module.exports = function (data, codeSystems, isCCD, CCDxml) {
    ...
    for (var i = 0; i < data.length; i++) {
        entriesArr[i] = libCCDAGen.getTimes(data[i]["smoking_statuses"][0]["date"])[1];
    ...
    for (var i = 0; i < uniqueEntries.length; i++) {
        var time = libCCDAGen.getTimes(data[i]["smoking_statuses"][0]["date"]);
        ...
        var xmlDoc = xmlDoc.node('entry').attr({
            ...
            .attr({
                displayName: data[i]["smoking_statuses"]["value"]
            })
            .attr({
