    ...
    for (var i = 0; i < data.length; i++) {
        ...
        var xmlDoc = xmlDoc.node('entry').attr({typeCode: "DRIV"})
            .node('observation').attr({classCode: "OBS"}).attr({moodCode: "EVN"})
                ...
                if (smoking) {
                    xmlDoc = libCCDAGen.code(xmlDoc, undefined, sec_entries_codes["SmokingStatusObservation"]);
                } else {
                    xmlDoc = libCCDAGen.code(xmlDoc, data[i].code);
                ...
                if (smoking) {
                    ...
                    xmlDoc = libCCDAGen.value(xmlDoc, undefined, "ST", undefined, data[i].value);
                }
            xmlDoc = xmlDoc.parent()
        .parent()
    }
