    ...
    for (var i = 0; i < data.length; i++) {
        ...
        xmlDoc = xmlDoc.node('entry').attr({typeCode: "DRIV"});
            xmlDoc = xmlDoc.node('substanceAdministration').attr({classCode: "SBADM"});
                ...
                xmlDoc = libCCDAGen.routeCode(xmlDoc, data[i]["administration"], true);
                if (data[i].administration && data[i].administration.body_site) {
                    libCCDAGen.code(xmlDoc, data[i].administration.body_site, undefined, 'approachSiteCode');
                }
                ...
                if (data[i].instructions) {
                    xmlDoc = libCCDAGen.entryRelationship(xmlDoc, data[i].instructions, 'act', 'SUBJ', '#immunSect', "2.16.840.1.113883.10.20.22.4.20");
                }
                if (data[i].refusal_reason) {
                    var rrData = {code: libCCDAGen.reverseTable("2.16.840.1.113883.5.8", data[i].refusal_reason)};
                    xmlDoc = libCCDAGen.entryRelationship(xmlDoc, rrData, 'observation', 'RSON', undefined, "2.16.840.1.113883.10.20.22.4.53");
                }
