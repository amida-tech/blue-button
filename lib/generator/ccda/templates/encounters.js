            ...
            .node('encounter')
                ...
                if (data[i]["encounter"]) {
                    xmlDoc = libCCDAGen.code(xmlDoc, data[i]["encounter"], {"code_system": "2.16.840.1.113883.6.12"});
                        ...
                        xmlDoc = libCCDAGen.code(xmlDoc, data[i]["encounter"] ? data[i]["encounter"]["translations"] : data[i]["encounter"]);
                ...
                xmlDoc = xmlDoc.node('performer')
                    .node('assignedEntity')
                        ...
                        xmlDoc = libCCDAGen.code(xmlDoc, data[i]["performers"] ? data[i]["performers"][0] : undefined).parent();
                    xmlDoc = xmlDoc.parent()
                .parent();
