...
module.exports = function (data, codeSystems, isCCD, CCDxml) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        if (dEncounter) {
            xmlDoc = libCCDAGen.code(xmlDoc, dEncounter, {"code_system": "2.16.840.1.113883.6.12"});
        ...
        xmlDoc = libCCDAGen.code(xmlDoc
            , dEncounter && dEncounter.translations ? dEncounter.translations[0] : dEncounter
            , {}, 'translation').parent();
        xmlDoc = xmlDoc.parent();
        xmlDoc = libCCDAGen.effectiveTime(xmlDoc, time); 
        xmlDoc = xmlDoc.node('performer').node('assignedEntity').node('id').attr({root: "PseduoMD-3"}).parent();
        var dPerformers = data[i].performers;
        var dPerformersCode = dPerformers && dPerformers[0] && dPerformers[0].code ? dPerformers[0].code[0] : undefined;
