...
var updateStatus = function(xmlDoc, entry, index) {
    ...
    var ob = er.node('observation').attr({classCode: "OBS"}).attr({moodCode: "EVN"}); // problem status observation
    ob.node('templateId').attr({root: "2.16.840.1.113883.10.20.22.4.6"});
                                
    libCCDAGen.id(ob, entry.identifiers);
    libCCDAGen.code(ob, undefined, sec_entries_codes["ProblemStatus"]);
                                
    ob.node('text').node('reference').attr({value: "#STAT" + (index + 1)});
    ob.node('statusCode').attr({code: "completed"});
...
module.exports = function (data, codeSystems, isCCD, CCDxml) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        ob.node('templateId').attr({root: "2.16.840.1.113883.10.20.22.4.4"});
        libCCDAGen.id(ob, data[i].identifiers);
        ob.node('text').node('reference').attr({value: "#problem" + (i + 1)});
        ob.node('statusCode').attr({code: "completed"});
        if (data[i].problem) {
