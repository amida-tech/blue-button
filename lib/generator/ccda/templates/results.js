                    ...
                    .node('observation').attr({classCode: "OBS"})
                        ...
                        .node('referenceRange')
                            .node('observationRange');
                                if (data[0]["results"][j]["referenceRange"] && data[0]["results"][j]["referenceRange"]["range"]) {
                                     xmlDoc = xmlDoc.node('text', data[0]["results"][j]["referenceRange"]["range"]).parent()
                                } else if (data[0]["results"][j]["referenceRange"]) {
                                    xmlDoc = xmlDoc.node('value').attr({"xsi:type": "IVL_PQ"})
                                        .node('low').attr({value: data[0]["results"][j]["referenceRange"]['low']})
                                            .attr({unit: data[0]["results"][j]["referenceRange"]['unit']}).parent()
                                        .node('high').attr({value: data[0]["results"][j]["referenceRange"]['high']})
                                            .attr({unit: data[0]["results"][j]["referenceRange"]['unit']}).parent()
                                    .parent()
                                }                               
