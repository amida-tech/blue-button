...
module.exports = function (data, codeSystems, isCCD, CCDxml) {
    ...
    for (var j = 0; j < data[0]["results"].length; j++) {
        ...
        xmlDoc = xmlDoc.node('component')
            ...
            .node('id').attr(results["identifiers"] !== undefined ? {
                root: results['identifiers'][0]['identifier']
            } : {
                nullFlavor: "UNK"
            }).parent();
        ...
        xmlDoc = xmlDoc.node('text')
            ...
            .node('value').attr({
                ...
                value: data[0]["results"][j]["value"]
            }).attr({
                unit: data[0]["results"][j]["unit"]
            }).parent()
            .node('interpretationCode')
            .attr(results["interpretations"] !== undefined ? {
                code: results["interpretations"][0].substr(0, 1)
            } : {
                nullFlavor: "UNK"
            })
            .attr({
