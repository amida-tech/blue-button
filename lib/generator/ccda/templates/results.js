...
var updateInterpretations = function (xmlDoc, interpretations) {
    if (interpretations) {
        interpretations.forEach(function (interpretation) {
            if (interpretation) {
                xmlDoc.node('interpretationCode').attr({
...
var updateEntry = function (xmlDoc, entry) {
    ...
    if (entry.results) {
        for (var j = 0; j < entry.results.length; j++) {
            ...
            var ob = comp.node('observation').attr({
                classCode: "OBS",
                moodCode: "EVN"
            });
            ...
            updateInterpretations(ob, results.interpretations);
            ...
            if (results.reference_range) {
                if (results.reference_range.range) {
                    ...
                    var v = or.node('value').attr({
                        "xsi:type": "IVL_PQ"
                    });
                    v.node('low').attr({
                        value: results.reference_range.low
                    })
                        .attr({
                            unit: results.reference_range.unit
                        });
                    v.node('high').attr({
                        value: results.reference_range.high
                    })
                        .attr({
                            unit: results.reference_range.unit
                        });
