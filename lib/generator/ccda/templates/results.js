    ...
    var xmlDoc = libCCDAGen.header(!isCCD ? doc : CCDxml, "2.16.840.1.113883.10.20.22.2.3", "2.16.840.1.113883.10.20.22.2.3.1", "30954-2", 
        ...
        var organizer = xmlDoc.node('entry').attr({typeCode: "DRIV"})
            ...
            organizer.node('templateId').attr({root: "2.16.840.1.113883.10.20.22.4.1"}).parent()
                ...
                xmlDoc = libCCDAGen.code(xmlDoc, data[0]["result_set"]);
                ...
                for (var j = 0; j < data[0]["results"].length; j++) {
                    ...
                    organizer.node('component')
                        .node('observation').attr({classCode: "OBS"}).attr({moodCode: "EVN"})
                            ...
                            .node('code').attr({code: results["result"]["code"] })
                                .attr({displayName: (results["result"]['name'] + "[#/volume] in Blood") })
                                .attr({codeSystem: codeSystems[results["result"]["code_system_name"]][0]})
                                .attr({codeSystemName: results["result"]["code_system_name"]}).parent()
                            .node('text')
