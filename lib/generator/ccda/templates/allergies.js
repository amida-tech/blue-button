    ...
    for (var i = 0; i < entries[0].length; i++) {
        ...
        var xmlDoc = xmlDoc.node('entry')
            ...
            .node('act')
                .attr({classCode: "ACT"})
                .attr({moodCode: "EVN"});
                // var comment = new libxmljs.Comment(CCDxml, "Allergy problem act");
                // comment.text();
                ...
                xmlDoc = libCCDAgen.code(xmlDoc, undefined, 
                    bbm.CCDA.sections_entries_codes["codes"]["AllergiesSection"]).parent();
                ...
                xmlDoc = xmlDoc.node('entryRelationship') // allergy observation
                    ...
                    .node('observation')
                        ...
                        xmlDoc = libCCDAgen.code(xmlDoc, undefined, 
                            bbm.CCDA.sections_entries_codes["codes"]["AllergyObservation"]).parent();
                ...
                for (var j = 0; j < entries[1][entries[0][i]]; j++) {
                    xmlDoc = xmlDoc.node('entryRelationship').attr({typeCode: "SUBJ"}).attr({inversionInd: "true"})
                        .node('observation').attr({classCode: "OBS"}).attr({moodCode: "EVN"}) // Allergy status xmlDoc
                            ...
                            xmlDoc = libCCDAgen.code(xmlDoc, undefined, 
                                bbm.CCDA.sections_entries_codes["codes"]["AllergyStatusObservation"]).parent();
