            ...
            .node('act')
                ...
                xmlDoc = xmlDoc.node('entryRelationship') // allergy observation
                    ...
                    .node('observation')
                        ...
                        xmlDoc = libCCDAgen.value(xmlDoc, data[i], "CD", (i + 1));
                    ...
                    if (data[i]["allergen"]) {
                        xmlDoc = xmlDoc.node('participant')
                            ...
                            .node('participantRole')
                                ...
                                .node('playingEntity')
                                    .attr({classCode: "MMAT"});
                                    xmlDoc = libCCDAgen.code(xmlDoc, data[i]["allergen"]);
                        ...
                        for (var j = 0; j < entries[1][entries[0][i]]; j++) {
                            xmlDoc = xmlDoc.node('entryRelationship').attr({typeCode: "SUBJ"}).attr({inversionInd: "true"})
                                .node('observation').attr({classCode: "OBS"}).attr({moodCode: "EVN"}) // Allergy status xmlDoc
                                    ...
                                    xmlDoc = libCCDAgen.value(xmlDoc, {"value": info[data[i]["status"]], "name": data[i]["status"]}, "CE");
                            ...
                            xmlDoc = xmlDoc.node('entryRelationship')
                                ...
                                .node('observation')
                                    ...
                                    xmlDoc = xmlDoc.node('value')
                                        .attr({"xsi:type": "CD"})
                                        .attr({code: data[i]["reaction"] ? data[i]["reaction"][0]["reaction"]["code"] : "UNK"})
                                        .attr({codeSystem: "2.16.840.1.113883.6.96"})
                                        .attr({displayName: data[i]["reaction"] ? data[i]["reaction"][0]["reaction"]["name"] : "UNK"}).parent();
