...
module.exports = function (data, codeSystems, isCCD, CCDxml) {
    ...
    for (var i = 0; i < entries[0].length; i++) {
        ...
        .node('id').attr({
            ...
            .attr({
                codeSystem: codeSystems["LOINC"][0]
            })
            .attr({
        ...
        if (data[i]["allergen"]) {
            xmlDoc = xmlDoc.node('participant').attr({
                ...
                .node('code').attr({
                    code: data[i]["allergen"]["code"]
                })
                .attr({
                    displayName: data[i]["allergen"]["name"]
                })
                .attr({
                    codeSystem: codeSystems[data[i]["allergen"]["code_system_name"]]
                })
                .attr({
                    codeSystemName: data[i]["allergen"]["code_system_name"]
                })
                .node('originalText')
                .node('reference').attr({
                    value: "#reaction" + (i + 1)
        ...
        for (var j = 0; j < entries[1][entries[0][i]]; j++) {
            ...
            .node('templateId').attr({
                ...
                .node('id').attr({
                    root: data[i]["identifiers"][0]["identifier"]
                }).parent()
                .node('code').attr({
                    nullFlavor: "NA"
                }).parent()
            ...
            xmlDoc = xmlDoc.node('value')
                ...
                .attr({
                    code: data[i]["reaction"] == undefined ? "UNK" : data[i]["reaction"][0]["reaction"]["code"]
                })
                .attr({
                    codeSystem: "2.16.840.1.113883.6.96"
                })
                .attr({
                    displayName: data[i]["reaction"] == undefined ? "UNK" : data[i]["reaction"][0]["reaction"]["name"]
                }).parent();
