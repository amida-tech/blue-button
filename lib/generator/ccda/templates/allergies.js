...
var updateAllergen = function(xmlDoc, allergen, index) {
    if (allergen) {
        ...
        var pr = p.node('participantRole').attr({classCode: "MANU"});
        var pe = pr.node('playingEntity').attr({classCode: "MMAT"});
        libCCDAgen.code(pe, allergen);
        pe.node('originalText').node('reference').attr({value: "#allergen" + (index + 1)});
    } else {
...
var updateStatus = function(xmlDoc, status) {
    if (status) {
        var er = xmlDoc.node('entryRelationship').attr({typeCode: "SUBJ", inversionInd: "true"});
        var ob = er.node('observation').attr({classCode: "OBS", moodCode: "EVN"});
        ob.node('templateId').attr({root: "2.16.840.1.113883.10.20.22.4.28"});
        libCCDAgen.code(ob, undefined, sec_entries_codes["AllergyStatusObservation"]);
...
var updateReaction = function(xmlDoc, reaction, i) { // Allergy reaction observation
    var es = xmlDoc.node('entryRelationship').attr({typeCode: "MFST", inversionInd: "true"});
    var ob = es.node('observation').attr({classCode: "OBS", moodCode: "EVN"});
                
    ob.node('templateId').attr({root: "2.16.840.1.113883.10.20.22.4.9"});

    ob.node('code').attr({nullFlavor: "NA"});
    ob.node('text').node('reference').attr({value: "#reaction" + (i + 1)});
    ob.node('statusCode').attr({code: 'completed'});
    libCCDAgen.effectiveTime(ob, libCCDAgen.getTimes(reaction["date_time"]));
    libCCDAgen.value(ob, reaction.reaction, "CD");
    var es2 = ob.node('entryRelationship').attr({typeCode: "SUBJ", inversionInd: "true"});
    var ob2 = es2.node('observation').attr({classCode: "OBS", moodCode: "EVN"});
    ob2.node('templateId').attr({root: "2.16.840.1.113883.10.20.22.4.8"});
    libCCDAgen.code(ob2, undefined, sec_entries_codes["SeverityObservation"]);
    ob2.node('text').node('reference').attr({value: "#severity" + (i + 4)});
    ob2.node('statusCode').attr({code: 'completed'});
    var dis = reaction.severity;
    if (dis) {
        libCCDAgen.value(ob2, dis.code, "CD");
        libCCDAgen.code(ob2, dis.interpretation, undefined, 'interpretationCode').parent();
    }
}
...
var updateSeverity = function(xmlDoc, severity, index) {
    if (severity) {
        var erAttrs = {typeCode: "SUBJ", inversionInd: "true"};
        var er = xmlDoc.node('entryRelationship').attr(erAttrs);
        var ob = er.node('observation').attr({classCode: "OBS", moodCode: "EVN"});
        ob.node('templateId').attr({root: "2.16.840.1.113883.10.20.22.4.8"});
        libCCDAgen.code(ob, undefined, sec_entries_codes["SeverityObservation"]);
        ob.node('text').node('reference').attr({value: "#severity" + (index + 1)});
        ob.node('statusCode').attr({code: 'completed'});
        libCCDAgen.value(ob, severity.code, "CD");
        libCCDAgen.code(ob, severity.interpretation, undefined, 'interpretationCode');
    }
}
...
var updateObservation = function(xmlDoc, observation, index) {
    if (observation) {
        ...
        ob.node('templateId').attr({root: "2.16.840.1.113883.10.20.22.4.7"}); // allergy intolerance observation
        libCCDAgen.id(ob, observation.identifiers);    
        libCCDAgen.code(ob, undefined, sec_entries_codes["AllergyObservation"]);
        ob.node('statusCode').attr({code: 'completed'});
...
var updateEntry = function(xmlDoc, entry, index) {
    ...
    var act = e.node('act').attr({classCode: "ACT", moodCode: "EVN"});
    act.node('templateId').attr({root: "2.16.840.1.113883.10.20.22.4.30"}); 
    libCCDAgen.id(act, entry.identifiers);
    libCCDAgen.code(act, undefined, sec_entries_codes["AllergiesSection"]);
    act.node('statusCode').attr({code: 'active'});
    if (entry.date_time) {
