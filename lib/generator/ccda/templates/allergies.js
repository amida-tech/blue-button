                    ...
                    organizer.node('templateId').attr({root: "2.16.840.1.113883.10.20.22.4.30"}).parent()
                        ...
                        for (var j = 0; j < entries[1][entries[0][i]]; j++) {
                            observation.node('entryRelationship').attr({typeCode: "SUBJ"}).attr({inversionInd: "true"})
                                ...
                                .parent()
                            .parent(); 
                            observation = observation.node('entryRelationship').attr({typeCode: "MFST"}).attr({inversionInd: "true"})
                                .node('observation').attr({classCode: "OBS"}).attr({moodCode: "EVN"}) // Allergy status observation
                                    ...
                                    observation = libCCDAgen.entryRelationship(observation, data, 'observation', i, j, "255604002", 1.1);
                                observation = observation.parent()
                            .parent();
                            observation = libCCDAgen.entryRelationship(observation, data, 'observation', i, j, data[i]["reaction"][0]["reaction"]["code"], 1.2);
                        }
                   observation.parent()
                .parent()   
            .parent()
