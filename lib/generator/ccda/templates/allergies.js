        ...
        "48765-2", "2.16.840.1.113883.6.1", undefined, undefined, "ALLERGIES, ADVERSE REACTIONS, ALERTS", isCCD);
            ...
            for (var i = 0; i < entries[0].length; i++) {
                ...
                var xmlDoc = xmlDoc.node('entry').attr({typeCode: "DRIV"})
                    ...
                    // allergy problem act
                    xmlDoc = xmlDoc.node('templateId').attr({root: "2.16.840.1.113883.10.20.22.4.30"}).parent()
                        ...
                        xmlDoc = xmlDoc.node('entryRelationship').attr({typeCode: "SUBJ"}).attr({inversionInd: "true"})
                            .node('observation').attr({classCode: "OBS"}).attr({moodCode: "EVN"})
                                .node('templateId').attr({root: "2.16.840.1.113883.10.20.22.4.7"}).parent() // allergy xmlDoc
                        ...
                        for (var j = 0; j < entries[1][entries[0][i]]; j++) {
                            xmlDoc = xmlDoc.node('entryRelationship').attr({typeCode: "SUBJ"}).attr({inversionInd: "true"})
                                ...
                                .parent()
                            .parent(); 
                            xmlDoc = xmlDoc.node('entryRelationship').attr({typeCode: "MFST"}).attr({inversionInd: "true"})
                                .node('observation').attr({classCode: "OBS"}).attr({moodCode: "EVN"}) // Allergy status xmlDoc
                            ...
                            .parent();
                            xmlDoc = libCCDAgen.entryRelationship(xmlDoc, data, 'observation', i, j, data[i]["reaction"][0]["reaction"] ? data[i]["reaction"][0]["reaction"]["code"] : "UNK", 1.2);
                        }
                   xmlDoc = xmlDoc.parent()
                .parent()   
            .parent()
