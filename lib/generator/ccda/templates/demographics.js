...
function writeXML(data, codeSystems) {
    var xml = {};
    xml = loadDemographics(data, codeSystems);
    return xml;
}


function loadDemographics(data, codeSystems) {
        ...
        .node('patient')
            ...
            .node('guardian')
                ...
                .node('addr').attr({use: "HP"})
                    ...
                    .node('country', data["addresses"][0]["country"]).parent()
                .parent()
                .node('telecom').attr({value: "tel:" + data["guardians"][0]["phone"][0]["number"]}).attr({use: "HP"}).parent()
                .node('guardianPerson')
                    .node('name')
