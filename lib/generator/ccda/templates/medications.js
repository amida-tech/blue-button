    ...
    for (var i = 0; i < data.length; i++) {
        ...
        .node('entryRelationship')
            ...
            .node('observation')
                ...
                .node('effectiveTime')
                    .node('low')
                        .attr(time? {value: time[0]} : {nullFlavor: "UNK"}).parent()
                .parent()
                .node('value')
                    .attr({"xsi:type": "CD"})
                    .attr({code: "233604007"})
            ...
            .node('entryRelationship').attr({typeCode: "REFR"})
                .node('supply').attr({classCode: "SPLY"}).attr({moodCode: "INT"})
                    ...
                    xmlDoc = xmlDoc.node('author')
                        ...
                        .node('assignedAuthor')
                            .node('id').attr(data[i]["product"]["identifiers"] == undefined ? {nullFlavor: "UNK"} : 
            ...
            xmlDoc = xmlDoc.node('entryRelationship').attr({typeCode: "REFR"})
                .node('supply').attr({classCode: "SPLY"}).attr({moodCode: "EVN"}) // medication dispense
                    ...
                    .node('id')
                        .attr({root: "1.2.3.4.56789.1"})
                        .attr({extension: "cb734647-fc99-424c-a864-7e3cda82e704"}).parent()
                    .node('statusCode').attr({code: "completed"}).parent()
