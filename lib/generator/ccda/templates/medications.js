        ...
        xmlDoc = xmlDoc.node('entry').attr({typeCode: "DRIV"})
            .node('substanceAdministration').attr({classCode: "SBADM"}).attr({moodCode: "EVN"})
                ...
                xmlDoc = xmlDoc.node('effectiveTime')
                    ...
                    .attr({operator: "A"})
                    .node('period').attr({value: data[i]["administration"]["interval"]["period"]["value"]})
                        .attr({unit: data[i]["administration"]["interval"]["period"]["unit"]}).parent()
                .parent()
                ...
                xmlDoc = xmlDoc.node('participant').attr({typeCode: "CSM"})
                    .node('participantRole').attr({classCode: "MANU"}) // drug vehicle
                        ...
                        .node('playingEntity').attr({classCode: "MMAT"});
                            xmlDoc = libCCDAGen.code(xmlDoc, data[i]["drug_vehicle"] ? data[i]["drug_vehicle"]["playingEntity"] : undefined).parent();
                            xmlDoc = xmlDoc.node('name', data[i]["drug_vehicle"] ? data[i]["drug_vehicle"]["playingEntity"]["name"] : "").parent()
                        .parent()
                    .parent()
                .parent()
                .node('entryRelationship') // Indication
                    ...
                    .node('observation').attr({classCode: "OBS"})
                        ...
                        xmlDoc = libCCDAGen.id(xmlDoc, data[i]["indication"]["identifiers"]);
                        ...
                        xmlDoc = libCCDAGen.effectiveTime(xmlDoc, libCCDAGen.getTimes(data[i]["indication"]["date_time"]));
                        xmlDoc = libCCDAGen.value(xmlDoc, data[i]["indication"]["value"], "CD");
                ...
                xmlDoc = xmlDoc.node('entryRelationship').attr({typeCode: "REFR"}) // Medication Dispense
                    .node('supply').attr({classCode: "SPLY"}).attr({moodCode: "EVN"}) 
                        ...
                        xmlDoc = libCCDAGen.id(xmlDoc, data[i]["dispense"]["identifiers"]);
