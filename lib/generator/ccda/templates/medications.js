...
var updateSupply = function (xmlDoc, entry, index) {
    ...
    if (entry.supply) {
        ...
        if (entry.supply.repeatNumber) {
            s.node('repeatNumber').attr({
                value: entry.supply.repeatNumber
            });
        }
        if (entry.supply.quantity) {
            s.node('quantity').attr({
                value: entry.supply.quantity
            });
        }
        ...
        if (entry.supply.author) {
            ...
            if (entry.supply.author.name) {
                ...
                libCCDAGen.addName(ap, entry.supply.author.name);
            }
        }
    }
    ...
    libCCDAGen.entryRelationship(s, entry.instructions, 'act', 'SUBJ', undefined, instERSetting); // Instructions
};
