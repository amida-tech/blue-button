                ...
                var organizer = xmlDoc.node('entry').attr({typeCode: "DRIV"})
                    ...
                    organizer.node('templateId').attr({root: "2.16.840.1.113883.10.20.22.4.16"}).parent()
                        ...
                        .node('consumable')
                                ...
                                .node('manufacturedMaterial')
                                    .node('code').attr({code: data[i]["product"]["product"]["code"]})
                                        ...
                                        .node('translation').attr({code: data[i]["product"]["product"]["translations"][0]["code"]})
                                                            ...
                                                            .attr({codeSystemName: data[i]["product"]["product"]["code_system_name"]}).parent()
                                    .parent()
                                .parent()
                            ...
                            .parent()
                        .parent();
                        organizer = libCCDAGen.performer(organizer, undefined, undefined, undefined, undefined, undefined, false, "notime");
                        organizer = organizer.node('participant').attr({typeCode: "CSM"})
                        ...
                        .node('entryRelationship').attr({typeCode: "REFR"})
                            .node('supply').attr({classCode: "SPLY"}).attr({moodCode: "INT"})
                                ...
                                .node('product')
                                    .node('manufacturedProduct').attr({classCode: "MANU"}) // medication information
                                        ...
                                        .node('manufacturedMaterial')
                                            .node('code').attr({code: data[i]["product"]["product"]["translations"][0]["code"]})
                                                         .attr({codeSystem: codeSystems[data[i]["product"]["product"]["translations"][0]["code_system_name"]]})
                                                         .attr({displayName: data[i]["product"]["product"]["translations"][0]["name"]})
                                            .node('originalText')
                                                .node('reference').attr({value: '#MedSec_1'}).parent()
                                            .parent()
                                            .node('translation').attr({code: data[i]["product"]["product"]["translations"][0]["code"]})
                                                                .attr({displayName: data[i]["product"]["product"]["translations"][0]["name"]})
                                                                .attr({codeSystem: codeSystems[data[i]["product"]["product"]["translations"][0]["code_system_name"]]})
                                                                .attr({codeSystemName: data[i]["product"]["product"]["translations"][0]["code_system_name"]}).parent()
                                            .parent()
                                        .parent()
                                        .node('manufacturerOrganization')
                                            .node('name', "Medication Factory Inc.").parent()
                                ...
                                organizer = libCCDAGen.performer(organizer, "2.16.840.1.113883.19.5.9999.456", "2981823", "generic", undefined, undefined, false, "notime");

                                organizer = organizer.node('author')
                            ...
                            .parent()
                        .parent()
                        organizer = organizer.node('entryRelationship').attr({typeCode: "REFR"})
                            .node('supply').attr({classCode: "SPLY"}).attr({moodCode: "EVN"}) // medication dispense
                                ...
                                .node('product')
                                    ...
                                    .parent()
                                .parent();
                                organizer = libCCDAGen.performer(organizer, "2.16.840.1.113883.19.5.9999.456", "2981823", "generic", undefined, undefined, true, undefined);
                            organizer = organizer.parent()
                        .parent();
                        organizer = libCCDAGen.precondition(organizer, data[i]["precondition"]);
