        ...
        if (data[i]["procedure_type"] == "procedure") {
            xmlDoc = xmlDoc.node('participant').attr({typeCode: "DEV"})
                .node('participantRole').attr({classCode: "MANU"})
                    ...
                    if (data[i].locations) {
                        if (data[i].locations[0]["location_type"]) {
                            xmlDoc = libCCDAGen.code(xmlDoc, data[i].locations[0]["location_type"]).parent();
                        }
                        xmlDoc = libCCDAGen.addr(xmlDoc, data[i].locations[0]["address"]);
                        xmlDoc = libCCDAGen.tel(xmlDoc, data[i].locations[0]["phone"]);
                        xmlDoc = libCCDAGen.playingEntity(xmlDoc, data[i].locations, {"classCode": "PLC"});
                    }
