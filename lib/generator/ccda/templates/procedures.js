    ...
    for (var i = 0; i < data.length; i++) {
        ...
        if (data[i]["procedure_type"] == "procedure") {
            xmlDoc = xmlDoc.node('specimen').attr({typeCode: "SPC"})
                ...
                .attr({displayName: data[i]["procedure"] ? data[i]["procedure"]["name"] : "UNK"}).parent()
                .parent()
                .parent()
                .parent();
        }
        xmlDoc = libCCDAGen.performerRevised(xmlDoc, undefined, {
            ...
            "extension": data[i]["identifiers"] ? data[i]["identifiers"][0]["extension"] : undefined,
        ...
        if (data[i]["procedure_type"] == "procedure") {
            ...
            xmlDoc = libCCDAGen.participant(xmlDoc, data[i]["locations"], {"playingEntity": 1}, {});
        }
