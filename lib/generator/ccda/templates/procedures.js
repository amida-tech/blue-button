...
module.exports = function(data, codeSystems, isCCD, CCDxml) {
    ...
    xmlDoc = xmlDoc.node('entry').attr({typeCode: "DRIV"})
        .node('procedure').attr({classCode: "PROC"}).attr({moodCode: "EVN"})
            .node('templateId').attr({root: "2.16.840.1.113883.10.20.22.4.14"}).parent() // smoking status observation
            ...
            .node('targetSiteCode').attr({code: data[i]["body_sites"][0]["code"]})
            ...
            .node('performer')
                .node('assignedEntity')
                    ...
                    .parent()
                    .node('telecom').attr({use: "WP"}).attr({value: data[i]["providers"][0]["phone"][0]["number"]}).parent()
                    .node('representedOrganization')
                        ...
                        .node('telecom').attr({use: "WP"})
                                        .attr({value: data[i]["providers"][0]["organization"]["phone"][0]["number"]}).parent()
                        .node('addr')
    ...
    .parent();
    i = 1;
    timeArr = libCCDAGen.getTimes(data[i]["date"]);
    xmlDoc = xmlDoc.node('entry')
        .node('observation').attr({classCode: "OBS"}).attr({moodCode: "EVN"})
            ...
            .node('code').attr({code: data[i]["procedure"]["code"]})
                         .attr({codeSystem: "2.16.840.1.113883.6.96"})
                         .attr({displayName: data[i]["procedure"]["name"]})
                         .attr({codeSystemName: data[i]["procedure"]["code_system_name"]})
                .node('originalText')
                    .node('reference').attr({value: "#Proc1"}).parent()
                .parent()
            .parent()
            ...
            .node('targetSiteCode').attr({code: data[i]["body_sites"][0]["code"]})
                                   .attr({codeSystem: "2.16.840.1.113883.6.96"})
                                   .attr({codeSystemName: "SNOMED CT"})
                                   .attr({displayName: data[i]["body_sites"][0]["name"]}).parent()
            .node('performer')
                .node('assignedEntity')
                    ...
                    xmlDoc.node('telecom').attr({use: "WP"}).attr({value: data[i]["providers"][0]["phone"][0]["number"]}).parent()
    ...
    xmlDoc = xmlDoc.node('entry')
        .node('act').attr({classCode: "ACT"}).attr({moodCode: "INT"})
            ...
            .node('id').attr({root: data[i]["identifiers"][0]["identifier"]})
                       .attr({extension: data[i]["identifiers"][0]["identifier_type"]}).parent()
            .node('code').attr({code: data[i]["procedure"]["code"]})
                         .attr({codeSystem: "2.16.840.1.113883.6.96"})
                         .attr({codeSystemName: "SNOMED CT"})
                         .attr({displayName: data[i]["procedure"]["name"]})
            ...
            xmlDoc = libCCDAGen.performer(xmlDoc, "2.16.840.1.113883.19", "1234", data[i]["providers"][0]["address"], 
                data[i]["providers"][0]["phone"], data[i]["providers"][0]["organization"]);
            xmlDoc = xmlDoc.node('participant').attr({typeCode: "LOC"})
                .node('participantRole').attr({classCode: "SDLOC"})
                    .node('templateId').attr({root: "2.16.840.1.113883.10.20.22.4.32"}).parent()
                    .node('code').attr({code: data[i]["locations"][0]["loc_type"]["code"]})
