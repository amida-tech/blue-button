        ...
        if (data[i]["procedure_type"] == "procedure") {
            xmlDoc = xmlDoc.node('specimen').attr({typeCode: "SPC"})
                .node('specimenRole').attr({classCode: "SPEC"});
                    xmlDoc = libCCDAGen.id(xmlDoc, libCCDAGen.attempt('data[i]["specimen"]["identifiers"]'));
                    xmlDoc = xmlDoc.node('specimenPlayingEntity');
                        xmlDoc = libCCDAGen.code(xmlDoc, libCCDAGen.attempt('data[i]["specimen"]["code"]')).parent();
                    xmlDoc = xmlDoc.parent()
                .parent()
            .parent();
        }

        xmlDoc = libCCDAGen.performerRevised(xmlDoc, libCCDAGen.attempt('data[i]["performer"][0]'));
