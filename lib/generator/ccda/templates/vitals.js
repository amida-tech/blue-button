    ...
    for (entry in entries[1]) {
        sum += entries[1][entry];
            ...
            for (var i = 0; i < entries[0].length; i++) {
                ...
                // console.log(i * entries[1][entries[0][i]]);
                var time = libCCDAGen.getTimes(data[i * (entries[1][entries[0][i]] - 1)]["date"]);
                        ...
                        for (var j = 0; j < entries[1][entries[0][i]]; j++) {
                            ...
                            xmlDoc = xmlDoc.node('component')
                                .node('observation').attr({classCode: "OBS"}).attr({moodCode: "EVN"})
                                    ...
                                    .node('effectiveTime').attr({value: time[0]}).parent()
                                    .node('value').attr({"xsi:type": "PQ"}).attr({value: data[count] ? data[count]["value"] : "UNK"}).attr({unit: data[j]["unit"]}).parent()
