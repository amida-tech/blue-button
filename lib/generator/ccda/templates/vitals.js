...
function writeXML(data, codeSystems) {
    var xml = {};
    xml = loadVitalSigns(data, codeSystems);
    return xml;
}

function loadVitalSigns(data, codeSystems) {
    ...
    for (var i = 0; i < uniqueEntries.length; i++) {
        numEntries[i] = entries[uniqueEntries[i]];
    }
    ...
    var xmlDoc = doc.node('ClinicalDocument').attr({"xmlns:xsi": "http://www.w3.org/2001/XMLSchema-instance"})
        ...
        .node('section')
            ...
            .node('code').attr({code: codeSystems[data[0]["vital"]["code_system_name"]][1] })
                .attr({codeSystem: codeSystems[data[0]["vital"]["code_system_name"]][0]})
                .attr({codeSystemName: data[0]["vital"]["code_system_name"]})
            ...
            for (var i = 0; i < uniqueEntries.length; i++) {
                ...
                var effectiveTime = data[i*numEntries[i]]["date"][0]["date"].split("-");
                        ...
                        for (var j = 0; j < entries[uniqueEntries[i]]; j++) {
                            organizer.node('component')
                                .node('observation').attr({classCode: "OBS"}).attr({moodCode: "EVN"})
                                    ...
                                    .node('effectiveTime').attr({value: time}).parent()
                                    .node('value').attr({"xsi:type": "PQ"}).attr({value: data[j+(i*numEntries[i == 0 ? i : (i-1)])]["value"]}).attr({unit: data[j]["unit"]}).parent()
