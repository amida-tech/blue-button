...
var entryRelationship = function (xmlDoc, data, type, i, j, code, subsection) {
    if (type == 'act') {
        ...
        .node('templateId').attr({root: "2.16.840.1.113883.10.20.22.4.8"}).parent()
            ...
            .node('text')
            .node('reference').attr({value: "#severity" +
            ...
            .attr({"xsi:type": "CD"})
            .attr({code: code})
            .attr({displayName: (subsection == 1.1 ? data[i]["reaction"] ? data[i]["reaction"][0]["severity"] :
...
var rateQuantity = function (xmlDoc, rate) {
    return rate ? xmlDoc.node('rateQuantity').attr({value: rate["value"]})
