...
var representedOrganization = function (xmlDoc, performer) {
    if (performer && performer != "generic") {
        xmlDoc = xmlDoc.node('representedOrganization')
            .node('id').attr({
                root: performer[0]["identifiers"][0]["identifier"]
            }).parent()
...
var code = function (xmlDoc, entity, spec) {
    ...
    if (entity == undefined) {
        ...
        return xmlDoc.node('code')
            .attr({
                code: spec["code"] ? spec["code"] : entity["code"]
            })
            .attr({
                displayName: spec["name"] ? spec["name"] : entity["name"]
            })
            .attr(entity["code_system_name"] ? {
                codeSystem: codeSystems[entity["code_system_name"]]
            } : {
                codeSystem: codeSystems["SNOMED CT"][0]
            })
            .attr({
                codeSystemName: spec["code_system_name"] ? spec["code_system_name"] : entity["code_system_name"]
            })
    }
...
var playingEntity = function (xmlDoc, nodes, spec) {
    spec = !spec ? {} : spec;
    xmlDoc = xmlDoc.node('playingEntity').attr({
        classCode: spec["classCode"] ? spec["classCode"] : "PLC"
    });
    xmlDoc = nodes["code"] ? code(xmlDoc, undefined, {}) : xmlDoc;
    xmlDoc = nodes["originalText"] ? xmlDoc.node('originalText')
        .node('reference').attr({
            value: value
        })
        .parent() : xmlDoc;
