...
var tel = function(xmlDoc, tel) {
    if (tel == undefined) {
        return xmlDoc.node('telecom').attr({nullFlavor: "NI"}).parent();
    } else {
...
var addr = function(xmlDoc, addr, birthplace) {
    if (addr == undefined) {
        xmlDoc = xmlDoc.node('addr').attr({nullFlavor: "NI"}).parent()
...
var getNumEntries = function(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        entriesArr[i] = data[i]["date"] !== undefined ? data[i]["date"][0]["date"].slice(0,4) : "UNK";
