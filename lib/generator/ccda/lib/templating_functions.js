        ...
        if (tel) {
            for (var j = 0; j < tel.length; ++j) {
                if (tel[j]) {
                    var telAttrs = {};
                    if (tel[j].number) {
                        telAttrs.value = "tel:" + tel[j].number;
                    }
                    if (tel[j].type) {
                        telAttrs.use = acronymize(tel[j].type);
                    }
                    xmlDoc.node('telecom').attr(telAttrs);
                }
            }
        }
        if (email) {
            for (var i = 0; i < email.length; ++i) {
                if (email[i] && email[i].address) {
                    ...
                    xmlDoc.node('telecom').attr(attrs);
                }
            }
        }
    } else {
        xmlDoc.node('telecom').attr({
            nullFlavor: "UNK"
        });
    }
    return xmlDoc;
...
var entryRelationship = function (xmlDoc, data, type, typeCode, ref, settings) {
    if (data) {
        var er = xmlDoc.node('entryRelationship').attr({
            typeCode: typeCode,
        });
        if (settings.inversionInd) {
            er.attr({
                inversionInd: settings.inversionInd
            });
        }
...
var indicationConstraint = function (xmlDoc, findings) {
    if (findings) {
        ...
        if (findings[0].date_time) {
            var time = getTimes(findings[0].date_time);
