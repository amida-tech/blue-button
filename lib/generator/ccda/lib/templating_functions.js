...
var insertObservation = function (xmlDoc, templateId, id, code, codeSystem, displayName, reference, timeArr, value, i) {
    return xmlDoc.node('entry').attr({
        ...
        .parent()
        .node('value', '1 pack per day').attr({
            "xsi:type": "ST"
        }).parent()
        .parent()
        .parent()
...
var header = function (doc, templateIdOptional, templateIdRequired, code, codeSystem, codeSystemName, displayName, title, isCCD) {
    ...
    if (templateIdRequired != undefined) {
        xmlDoc = xmlDoc.node('templateId').attr({
            root: templateIdRequired
        }).parent();
    }

    if (codeSystemName != undefined) {
        xmlDoc = xmlDoc.node('code').attr({
            code: code
        })
            .attr({
...
var tel = function (xmlDoc, tel) {
    if (tel) {
        return xmlDoc.node('telecom').attr({
            value: checkObj(tel, tel[0]["number"])
        }).attr(tel[0]["use"] ? {use: acronymize(tel[0]["use"])} : {use: "HP"}).parent()
    } else {
        return xmlDoc.node('telecom').attr({
            nullFlavor: "UNK"
        }).parent();
...
var addr = function (xmlDoc, addr, birthplace) {
    if (addr == undefined) {
        ...
        xmlDoc = xmlDoc.node('addr');
        if (addr[0]["use"]) {
            xmlDoc = xmlDoc.attr({use: acronymize(addr[0]["use"])});
        }
...
var performerRevised = function (xmlDoc, performer, spec) {
    ...
    if (spec && spec["typeCode"]) {
        xmlDoc = xmlDoc.node('performer').attr({typeCode: spec["typeCode"]});
    } else {
        xmlDoc = xmlDoc.node('performer');
    }
    if (spec && spec["tID"])
        xmlDoc = xmlDoc.node('templateId').attr({root: "2.16.840.1.113883.10.20.22.4.87"}).parent();
...
var manufacturedProduct = function (xmlDoc, data, i, ref) {
    xmlDoc = xmlDoc.node('manufacturedProduct').attr({
        classCode: "MANU"
    ...
    if (data[i]["product"]) {
        xmlDoc = xmlDoc.node('id').attr(data[i]["product"]["identifiers"] == undefined ? {
            nullFlavor: "UNK"
        } : {
...
var entryRelationship = function (xmlDoc, data, type, i, j, code, subsection) {
    if (type == 'act') {
        ...
        xmlDoc.node('entryRelationship').attr({
            typeCode: "SUBJ"
        }).attr({
            inversionInd: "true"
        })
        ...
        .node('templateId').attr({
            ...
            .node('reference').attr({
                value: "#severity" +
                    (subsection == 1.1 ? (j + 4 + i) : ((i == 0) ? (j + i + 2) : (j + i + 1)))
            }).parent()
            .parent()
            .node('statusCode').attr({
                code: 'completed'
            ...
            .attr({
                displayName: (subsection == 1.1 ? data[i]["reaction"] ? data[i]["reaction"][0]["severity"] :
                    "UNK" : data[i]["reaction"] ? data[i]["reaction"][0]["reaction"]["name"] :
                    "UNK")
            })
            .attr({
                codeSystem: "2.16.840.1.113883.6.96"
            ...
            .attr({
                code: (subsection == 1.1 ? "S" : "N")
            })
            .attr({
                displayName: (subsection == 1.1 ? 'Susceptible' : "Normal")
            })
            .attr({
...
var substanceAdministration = function (xmlDoc, data, i, refSBADM, refCONS, templIdSBADM, templIdCONS) {
    ...
    xmlDoc = xmlDoc.node('substanceAdministration').attr({
        classCode: "SBADM"
    }).attr({
        moodCode: "EVN"
    }).attr({
...
var id = function (xmlDoc, id, ext) {
    if (id) {
        if (ext) {
            return xmlDoc.node('id').attr({
                    root: checkObj(id, id[0]["identifier"])
                })
                .attr({
                    extension: checkObj(ext, ext[0]["identifier_type"])
                }).parent()
        } else {
            xmlDoc = xmlDoc.node('id').attr({
                root: checkObj(id, id[0]["identifier"])
            });
            if (id[0]["identifier_type"]) {
                xmlDoc = xmlDoc.attr({extension: id[0]["identifier_type"]})
            }
            xmlDoc = xmlDoc.parent();
            return xmlDoc;
        }
...
var routeCode = function (xmlDoc, administration) {
    if (administration == undefined) {
        return xmlDoc.node('routeCode').attr({
            nullFlavor: "UNK"
        }).parent()
    } else if (administration["body_site"] != undefined) {
        return xmlDoc.node('routeCode').attr({
                code: administration["body_site"]['code']
            })
            .attr({
                codeSystem: "2.16.840.1.113883.3.26.1.1"
            })
            .attr({
                codeSystemName: administration["body_site"]["code_system_name"]
            })
            .attr({
                displayName: administration["body_site"]["name"]
            }).parent();
    } else if (administration["route"] != undefined) {
        return xmlDoc.node('routeCode').attr({
                code: administration['route']['code']
            })
            .attr({
                codeSystem: "2.16.840.1.113883.3.26.1.1"
            })
            .attr({
                codeSystemName: administration["route"]["code_system_name"]
            })
            .attr({
                displayName: administration["route"]["name"]
            }).parent()
    } else {
        return xmlDoc;
    }
...
var doseQuantity = function (xmlDoc, administration) {
    if (administration !== undefined) {
        if (administration["body_site"] !== undefined && administration["body_site"]["dose"] != undefined) {
            return xmlDoc.node('doseQuantity').attr({
...
var medication_administration = function (xmlDoc, administration) {
    if (administration == undefined) {
        return xmlDoc.node('routeCode').attr({
            ...
            .node('doseQuantity').attr({
                nullFlavor: "UNK"
            }).parent()
            .node('rateQuantity').attr({
                nullFlavor: "UNK"
            }).parent()
            .node('maxDoseQuantity').attr({
                nullFlavor: "UNK"
            })
            .node('numerator').attr({
                nullFlavor: "UNK"
            }).parent()
            .node('denominator').attr({
                nullFlavor: "UNK"
            }).parent()
            .parent()
            .node('administrationUnitCode').attr({
                nullFlavor: "UNK"
            }).parent()
    } else {
        ...
        xmlDoc = xmlDoc.node('maxDoseQuantity').attr({
            nullFlavor: "UNK"
        })
            .node('numerator').attr({
                nullFlavor: "UNK"
        ...
        if (administration["form"] != undefined) {
            xmlDoc = xmlDoc.node('administrationUnitCode').attr({
                code: administration["form"]["code"]
            })
                .attr({
                    displayName: administration["form"]["name"]
                })
                .attr({
                    codeSystem: "2.16.840.1.113883.3.26.1.1"
                })
                .attr({
                    codeSystemName: administration["form"]["code_system_name"]
                }).parent();
        }
        return xmlDoc;
    }
...
var code = function (xmlDoc, entity, spec) {
    spec = !spec ? {} : spec;
    if (entity == undefined) {
        return xmlDoc.node('code').attr({
            nullFlavor: "UNK"
        });
    } else {
        xmlDoc = xmlDoc.node('code');
        var code_attr = spec["code"] ? spec["code"] : entity["code"];
        if (code_attr) {
            xmlDoc = xmlDoc.attr({
                code: code_attr
            });
        }
        var name_attr = spec["name"] ? spec["name"] : entity["name"];
        if (name_attr) {
            xmlDoc = xmlDoc.attr({
                displayName: spec["name"] ? spec["name"] : entity["name"]
            });  
        } 
        var code_system_attr = entity["code_system_name"] ? codeSystems[entity["code_system_name"]] : codeSystems["SNOMED CT"][0];
        if (code_system_attr) {
            xmlDoc = xmlDoc.attr({codeSystem: code_system_attr});
        }

        var code_system_name_attr = spec["code_system_name"] ? spec["code_system_name"] : entity["code_system_name"];
        if (code_system_name_attr) {
            xmlDoc = xmlDoc.attr({
                codeSystemName: code_system_name_attr 
            });
        }
        return xmlDoc;
    }
...
var rateQuantity = function (xmlDoc, rate) {
    return rate ? xmlDoc.node('rateQuantity').attr({
            value: rate["value"]
        })
        .attr({
            unit: rate["unit"]
        }).parent() : unk(xmlDoc, 'rateQuantity', 'UNK');
}
...
var immunization_medication_information = function (xmlDoc, product) {
    return xmlDoc.node('manufacturedProduct').attr({
            classCode: "MANU"
        }) // medication information
        .node('templateId').attr({
            root: templateId
        ...
        .node('manufacturedMaterial')
        .node('code').attr({
            code: data[i]["product"]["product"]["code"]
        })
        .attr({
            codeSystem: codeSystems[data[i]["product"]["product"]["code_system_name"]]
        })
        .attr({
            displayName: data[i]["product"]["product"]["name"]
        })
        .attr({
            codeSystemName: data[i]["product"]["product"]["code_system_name"]
        })
        .node('originalText')
        .node('reference').attr({
            value: ref
        ...
        .parent()
        .node('translation').attr({
            code: data[i]["product"]["product"]["translations"][0]["code"]
        })
        .attr({
            displayName: data[i]["product"]["product"]["translations"][0]["name"]
        })
        .attr({
            codeSystemName: data[i]["product"]["product"]["translations"][0]["code_system_name"]
        })
        .attr({
            codeSystem: codeSystems[data[i]["product"]["product"]["translations"][0]["code_system_name"]]
        }).parent()
        .parent()
...
var maritalStatusCode = function (xmlDoc, marriage_status) {
    if (marriage_status == undefined) {
        return xmlDoc.node('maritalStatusCode').attr({
            nullFlavor: "UNK"
        }).parent();
    } else {
        return xmlDoc.node('maritalStatusCode').attr({
...
var languageCommunication = function (xmlDoc, languages) {
    if (languages == undefined) {
        return xmlDoc.node('languageCommunication')
            ...
            .node('proficiencyLevelCode').attr({
                nullFlavor: "UNK"
            }).parent()
            .node('preferenceInd').attr({
                nullFlavor: "UNK"
            }).parent()
            .parent()
    } else {
        ...
        if (languages[0]["proficiency"] != undefined) {
            xmlDoc = xmlDoc.node('proficiencyLevelCode').attr({
        ...
        xmlDoc = xmlDoc.node('preferenceInd').attr({
            value: "true"
        }).parent()
            .parent()
        return xmlDoc;
...
var translation = function (xmlDoc, translations) {
    if (translations == undefined) {
        return xmlDoc.node('translation').attr({
            nullFlavor: "UNK"
        }).parent();
    } else {
        return xmlDoc.node('translation').attr({
            ...
            .attr({
                codeSystemName: translations[0]["code_system_name"]
            }).parent()
    }
}
...
var participant = function (xmlDoc, locations, nodes, spec) {
    if (locations == undefined) {
        return xmlDoc.node('participant').attr({
            nullFlavor: "UNK"
        }).parent();
    } else {
        ...
        if (locations[0]["loc_type"] && nodes["code"]) {
            xmlDoc = code(xmlDoc, locations[0]["loc_type"], {
...
var indicationConstraint = function (xmlDoc, findings, time) {
    if (findings == undefined) {
        ...
        return xmlDoc.node('entryRelationship').attr({
                typeCode: "RSON"
            })
            .node('observation').attr({
                classCode: "OBS"
...
var precondition = function (xmlDoc, precondition) {
    if (precondition == undefined) {
        ...
        return xmlDoc.node('precondition').attr({
                typeCode: "PRCN"
