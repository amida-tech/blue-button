...
var attempt = function(data, i, field) {
    try {
        return eval(field) ? eval(field) : "UNK";     
    } catch(err) {
        return "UNK";
    }
};
...
var representedOrganization = function (xmlDoc, performer, tag) {
    ...
    if (performer && performer != "generic") {
        xmlDoc = xmlDoc.node('representedOrganization');
            ...
            xmlDoc = xmlDoc.node('name', performer["name"][0]).parent();
    ...
    xmlDoc = xmlDoc.node('manufacturedProduct').attr({classCode: "MANU"}) // medication information
        ...
        if (product && product["product"]) {
            ...
            xmlDoc = xmlDoc.node('manufacturedMaterial');
                xmlDoc = code(xmlDoc, sec ? product["product"] ? product["product"]["translations"][0] : undefined : product["product"], {});
                xmlDoc = xmlDoc.node('originalText')
                    .node('reference').attr({value: ref}).parent()
                .parent();
                xmlDoc = code(xmlDoc, product["product"] ? 
                    product["product"]["translations"][0] : undefined, {}, 'translation').parent();
                xmlDoc = xmlDoc.parent() // end code
                if (product["lot_number"]) {
                    xmlDoc = xmlDoc.node('lotNumberText', product["product"]["lot_number"]).parent();
                }
...
var participant = function (xmlDoc, locations, nodes, spec) {
    if (!locations) {
        ...
        if (locations[0]["location_type"] && nodes["code"]) {
