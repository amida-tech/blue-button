...
var performerRevised = function (xmlDoc, performer, spec) {
    ...
    if (spec && spec["typeCode"]) {
        xmlDoc = xmlDoc.node('performer').attr({
    ...
    if (id) {
        if (ext) {
            ...
            if (id[0]["identifier_type"]) {
                xmlDoc = xmlDoc.attr({
                    extension: id[0]["identifier_type"]
                })
            }
            xmlDoc = xmlDoc.parent();
            return xmlDoc;
...
var effectiveTime = function (xmlDoc, time, tag) {
    ...
    if (time) {
        xmlDoc = xmlDoc.node(node).node('low').attr({value: time[0]}).parent();
        if (time[2]) {
        ...
        xmlDoc = xmlDoc.node(node).node('low').attr({nullFlavor: "UNK"}).parent();
        return xmlDoc;
    }
}
