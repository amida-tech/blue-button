            ...
            .node('observation').attr({classCode: "OBS"}).attr({moodCode: "EVN"}) // Severity Observation
                ...
                .node('text')
                    .node('reference').attr({value: "#severity" + 
                        (subsection == 1.1 ? (j+ 4 + i) : ((i == 0) ? (j + i + 2) : (j + i + 1)) ) }).parent()
                .parent()
                .node('statusCode').attr({code: 'completed'}).parent()
                .node('value')
                    .attr({"xsi:type": "CD"})
...
var code = function(xmlDoc, entity, spec) {
    ...
    if (entity == undefined) {
        ...
        return xmlDoc.node('code')
                    .attr({code: spec["code"] ? spec["code"] : entity["code"] })
                    .attr({displayName: spec["name"] ? spec["name"] : entity["name"]})
                    .attr(entity["code_system_name"]? {codeSystem: codeSystems[entity["code_system_name"]]} : {codeSystem: codeSystems["SNOMED CT"][0]})
                    .attr({codeSystemName: spec["code_system_name"] ? spec["code_system_name"] : entity["code_system_name"]})   
    }
...
var participant = function(xmlDoc, locations, nodes, spec) {
    if (locations == undefined) {
        ...
        xmlDoc = xmlDoc.node('participant').attr({typeCode: spec["typeCode"] ? spec["typeCode"] : "LOC"})
            .node('participantRole').attr({classCode: spec["classCode"] ? spec["classCode"] : "SDLOC"}) // service delivery location template
                .node('templateId').attr({root: spec["tId"] ? spec["tId"] : "2.16.840.1.113883.10.20.22.4.32"}).parent();
...
var playingEntity = function(xmlDoc, nodes, spec) {
    spec = !spec ? {} : spec;
    xmlDoc = xmlDoc.node('playingEntity').attr({classCode: spec["classCode"] ? spec["classCode"] : "PLC"});
        xmlDoc = nodes["code"] ? code(xmlDoc, undefined, {}) : xmlDoc;
        xmlDoc = nodes["originalText"] ? xmlDoc.node('originalText')
            .node('reference').attr({value: value})
        .parent() : xmlDoc;
